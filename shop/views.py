...
def home(request):
    ...
    return render(request, "home.html", {'products':products, 'cartItems':cartItems})
...
def product_view(request, myid):
    ...
    if request.method=="POST":
        ...
        review = Review(content=content, product=product)
        review.save()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if u.check_password(current_password):
                ...
                u.save()
                alert = True
                return render(request, "customer.html", {'alert':alert})
            else:
                currpasswrong = True
                return render(request, "customer.html", {'currpasswrong':currpasswrong})
        except:
            pass
    return render(request, "customer.html", {'cartItems':cartItems})